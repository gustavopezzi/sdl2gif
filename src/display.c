#include "display.h"

static SDL_Window* window = NULL;
static SDL_Renderer* renderer = NULL;

static uint32_t* framebuffer = NULL;

static SDL_Texture* framebuffer_texture = NULL;

static uint16_t window_width = 600;
static uint16_t window_height = 600;

static bool recording_gif = false;

bool init_window(void) {
  if (SDL_Init(SDL_INIT_EVERYTHING) != 0) {
    fprintf(stderr, "Error initializing SDL.\n");
    return false;
  }

  window = SDL_CreateWindow("voxelspace", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, window_width, window_height, 0);
  if (!window) {
    fprintf(stderr, "Error creating SDL window.\n");
    return false;
  }

  renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_PRESENTVSYNC);
  if (!renderer) {
    fprintf(stderr, "Error creating SDL renderer.\n");
    return false;
  }

  framebuffer = malloc(sizeof(uint32_t) * SCREEN_WIDTH * SCREEN_HEIGHT);
  
  framebuffer_texture = SDL_CreateTexture(
    renderer,
    SDL_PIXELFORMAT_RGBA32,
    SDL_TEXTUREACCESS_STREAMING,
    SCREEN_WIDTH,
    SCREEN_HEIGHT
  );

  return true;
}

void start_recording_gif(void) {
  recording_gif = true;
}

void stop_recording_gif(void) {
  recording_gif = false;
}

void draw_pixel(uint16_t x, uint16_t y, uint32_t color) {
  if (x < 0 || x >= SCREEN_WIDTH || y < 0 || y >= SCREEN_HEIGHT) {
    return;
  }
  framebuffer[(SCREEN_WIDTH * y) + x] = color;
}

void draw_line(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint32_t color) {
  int16_t delta_x = (x1 - x0);
  int16_t delta_y = (y1 - y0);

  uint16_t longest_side_length = (abs(delta_x) >= abs(delta_y)) ? abs(delta_x) : abs(delta_y);

  float x_inc = delta_x / (float)longest_side_length;
  float y_inc = delta_y / (float)longest_side_length;

  float current_x = x0;
  float current_y = y0;

  for (size_t i = 0; i <= longest_side_length; i++) {
    draw_pixel(round(current_x), round(current_y), color);
    current_x += x_inc;
    current_y += y_inc;
  }
}

void draw_rect(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint32_t color) {
  for (size_t i = 0; i < width; i++) {
    for (size_t j = 0; j < height; j++) {
      uint16_t current_x = x + i;
      uint16_t current_y = y + j;
      draw_pixel(current_x, current_y, color);
    }
  }
}

void render_framebuffer(void) {
  SDL_UpdateTexture(
    framebuffer_texture,
    NULL,
    framebuffer,
    (int)(SCREEN_WIDTH * sizeof(uint32_t))
  );
  SDL_RenderCopy(renderer, framebuffer_texture, NULL, NULL);
  SDL_RenderPresent(renderer);

  if (recording_gif) {
    static int numframes = 0;
    int format = SDL_PIXELFORMAT_BGR888;
    SDL_Surface *surface = SDL_CreateRGBSurfaceWithFormatFrom(framebuffer, 240, 240, SDL_BITSPERPIXEL(format), 240 * SDL_BYTESPERPIXEL(format), format);
    SDL_LockSurface(surface);
    char filename[64];
    sprintf(filename, "./frames/frame-%03d.bmp", numframes++);
    SDL_SaveBMP(surface, filename);
    SDL_UnlockSurface(surface);
    SDL_FreeSurface(surface);
  }
}

void clear_framebuffer(uint32_t color) {
  for (size_t i = 0; i < SCREEN_WIDTH * SCREEN_HEIGHT; i++) {
    framebuffer[i] = color;
  }
}

void destroy_window(void) {
  free(framebuffer);
  SDL_DestroyRenderer(renderer);
  SDL_DestroyWindow(window);
  SDL_Quit();
}